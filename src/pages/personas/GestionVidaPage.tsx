...
import { CurrencyCode, VentaVida, VentaVidaMutationInput, VentaVidaStatus } from "../../types/models";
import { VentasVidaService } from "../../services/ventasVida.service";
import { VentasVidaTable } from "../../components/tables/VentasVidaTable";
import { VentaVidaFormDrawer } from "../../components/forms/VentaVidaFormDrawer";
...
export const GestionVidaPage = () => {
  const { user } = useAuth();
  const uid = user?.id ?? "";
  const [data, setData] = useState<VentaVida[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState(() => getStored(`pvida_search_${uid}`, ""));
  const [dateFrom, setDateFrom] = useState(() => getStored(`pvida_dateFrom_${uid}`, ""));
  const [dateTo, setDateTo] = useState(() => getStored(`pvida_dateTo_${uid}`, ""));
  const [statusFilter, setStatusFilter] = useState<string[]>(() => getStored(`pvida_statuses_${uid}`, []));
  const [monedaFilter, setMonedaFilter] = useState<CurrencyCode | "all">(() => getStored(`pvida_moneda_${uid}`, "all"));
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editTarget, setEditTarget] = useState<VentaVida | undefined>(undefined);
  const [deleteTarget, setDeleteTarget] = useState<string | null>(null);

  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const result = await VentasVidaService.getAll();
      setData(result);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (!uid) return;
    localStorage.setItem(`pvida_search_${uid}`, JSON.stringify(search));
  }, [search, uid]);

  useEffect(() => {
    if (!uid) return;
    localStorage.setItem(`pvida_dateFrom_${uid}`, JSON.stringify(dateFrom));
  }, [dateFrom, uid]);

  useEffect(() => {
    if (!uid) return;
    localStorage.setItem(`pvida_dateTo_${uid}`, JSON.stringify(dateTo));
  }, [dateTo, uid]);

  useEffect(() => {
    if (!uid) return;
    localStorage.setItem(`pvida_statuses_${uid}`, JSON.stringify(statusFilter));
  }, [statusFilter, uid]);

  useEffect(() => {
    if (!uid) return;
    localStorage.setItem(`pvida_moneda_${uid}`, JSON.stringify(monedaFilter));
  }, [monedaFilter, uid]);
  ...
  const handleCreate = async (input: VentaVidaMutationInput): Promise<boolean> => {
    try {
      await VentasVidaService.create(input);
      await loadData();
      return true;
    } catch {
      return false;
  ...
  const handleEdit = async (input: VentaVidaMutationInput): Promise<boolean> => {
    if (!editTarget) return false;
    try {
      await VentasVidaService.update(editTarget.id, input);
      await loadData();
      return true;
    } catch {
      return false;
  ...
  const handleDelete = async () => {
    ...
    await VentasVidaService.remove(deleteTarget);
    setDeleteTarget(null);
    await loadData();
  };

  const openEdit = (item: VentaVida) => {
    setEditTarget(item);
    setIsFormOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Gestión Vida</h1>
