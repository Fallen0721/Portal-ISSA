...
interface VentaSaludFormDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (input: VentaSaludMutationInput) => Promise<boolean> | boolean;
  initialData?: VentaSalud;
}

const emptyForm: VentaSaludMutationInput = {
  no: null,
...
export const VentaSaludFormDrawer = ({
  isOpen,
  onClose,
  onSubmit,
  initialData,
}: VentaSaludFormDrawerProps) => {
  const [formData, setFormData] = useState<VentaSaludMutationInput>(emptyForm);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [productos, setProductos] = useState<CatalogItem[]>([]);
  const [ramos, setRamos] = useState<CatalogItem[]>([]);

  useEffect(() => {
    ProductosService.getAll("salud").then(setProductos).catch(() => setProductos([]));
  ...
  const handleChange = <K extends keyof VentaSaludMutationInput>(
    field: K,
    value: VentaSaludMutationInput[K],
  ) => {
  ...
  const companiaOptions = COMPANIAS_SALUD.map((c) => ({ value: c, label: c }));
  ...
  return (
    <Drawer
      isOpen={isOpen}
      onClose={onClose}
      title={initialData ? "Editar Gestión Salud" : "Nueva Gestión Salud"}
      width="lg"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <SelectField
            ...
            onChange={(e) =>
              handleChange("moneda", e.target.value as VentaSalud["moneda"])
            }
            options={[
