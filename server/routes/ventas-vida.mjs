...
export const ventasVidaRouter = Router();

const ALLOWED_ROLES = ["admin", "personas", "gerente_comercial"];

ventasVidaRouter.get(
  "/",
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await getPool().query(
      "SELECT * FROM ventas_vida ORDER BY fecha_ingreso DESC",
    );
...
ventasVidaRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await getPool().query(
      "SELECT * FROM ventas_vida WHERE id = ? LIMIT 1",
      [req.params.id],
    );
    const row = rows[0];
    if (!row) return res.status(404).json(null);

    const registro = mapVentaVida(row);
...
ventasVidaRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json(mapVentaVida(rows[0]));
  }),
);

ventasVidaRouter.put(
  "/:id",
  asyncHandler(async (req, res) => {
    if (!ALLOWED_ROLES.includes(req.user.role)) {
      return sendError(res, 403, "No autorizado");
    ...
    const current = mapVentaVida(rows[0]);
    ...
    res.json(mapVentaVida(updatedRows[0]));
  }),
);

ventasVidaRouter.delete(
  "/:id",
  asyncHandler(async (req, res) => {
    if (!ALLOWED_ROLES.includes(req.user.role)) {
      return sendError(res, 403, "No autorizado");
    ...
    const registro = mapVentaVida(rows[0]);
    if (!canManageRecord(req.user, registro.ownerUserId)) {
      return sendError(res, 403, "No autorizado");
    }

    await getPool().query("DELETE FROM ventas_vida WHERE id = ?", [req.params.id]);
    res.json({ ok: true });
  }),
);
